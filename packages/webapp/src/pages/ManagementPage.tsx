...
function DIDVoicePicker({ voiceId, voiceUrl, onChange }: DIDVoicePickerProps) {
  const { t } = useI18n();
  const [tab, setTab] = useState<"official" | "uploads">(voiceUrl ? "uploads" : "official");
...
function HeyGenAvatarPicker({ avatarId, avatarPhotoUrl, onChange }: HeyGenAvatarPickerProps) {
  const { t } = useI18n();
    ...
    <div className="voice-picker">
      ...
      {tab === "uploads" &&
        (uploadsLoading ? (
          ...
          <div className="avatar-picker__grid">
            {uploads.map((upload) => {
              const isSelected = avatarPhotoUrl === upload.url;
              return (
                <div
                  key={upload.id}
                  className={`avatar-picker__item${isSelected ? " avatar-picker__item--selected" : ""}`}
