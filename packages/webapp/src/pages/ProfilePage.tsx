...
function GalleryTab() {
  ...
  const load = useCallback((sec: Section, pg: number, folderId: string | null) => {
    setLoading(true);
    setError(null);
    api.gallery
      .list({
  ...
  useEffect(() => {
    load(section, page, activeFolderId);
  }, [section, page, activeFolderId, load]);
    ...
    async (job: GalleryJob) => {
      ...
      if (isFav && favFolder) {
        ...
        if (activeFolderId === favFolder.id) {
          // Viewing the Favorites folder — item must disappear from list
        ...
        if (!favAlreadyExists) {
          // Favorites folder was just created — reload to get full info
          loadFolders();
        }
      }
    },
