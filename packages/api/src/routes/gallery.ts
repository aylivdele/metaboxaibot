...
export const galleryRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.get<{
    Querystring: { section?: string; page?: string; limit?: string; folderId?: string };
  }>("/gallery", async (request) => {
    const userId = (request as AuthRequest).userId;
    const { section, page = "1", limit = "20", folderId } = request.query;
   ...
   * For the Favorites folder (isDefault=true), auto-creates it on first use.
   */
  fastify.post<{
    Params: { folderId: string };
    Body: { jobId: string };
